...
import { HERO, INTRO, STATEMENT, PRINCIPLES, CTA, SITE } from "@/content/site";

export default function Home() {
  return (
    <>
      {/* ---- Hero: full-bleed, the headline sitting in the dark of the eave ---- */}
      <section className="relative h-[92svh] min-h-[560px] w-full overflow-hidden">
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2">
          <div className="h-10 w-px animate-pulse bg-bg/50" />
        </div>
      </section>
      ...
      {/* ---- The work ---- */}
      <section id="work" className="border-t border-line bg-bg-lift scroll-mt-20">
        <div className="shell py-28 md:py-36">
          <Reveal className="mb-16 max-w-2xl">
            ...
            <p className="mt-8 max-w-xl text-lg text-ink-soft">
              Open a row for the problem it was built against, what it does,
              and the decisions behind it. {LIVE_COUNT} of the {TOTAL_COUNT} are
              in production today.
            </p>
          </Reveal>

          {GROUPS.map((group, gi) => (
            <div key={group.id} className={gi === 0 ? "" : "mt-24 md:mt-28"}>
              <Reveal className="mb-8 max-w-xl">
                <h3 className="text-xl md:text-2xl">{group.label}</h3>
                <p className="mt-2 text-ink-soft">{group.note}</p>
              </Reveal>
              <WorkIndex
                group={group}
                startAt={
                  GROUPS.slice(0, gi).reduce((n, g) => n + g.items.length, 0) + 1
                }
              />
            </div>
          ))}
      ...
      <section
        id="approach"
        className="scroll-mt-20 border-t border-line bg-bg-tint"
      >
        <div className="shell py-28 md:py-36">
      ...
      <section id="stack" className="scroll-mt-20 border-t border-line">
        <div className="shell py-28 md:py-36">
          ...
          <Reveal>
            <dl className="grid md:grid-cols-2 md:gap-x-16">
              {STACK.map(([label, body]) => (
                <div
                  key={label}
                  className="grid border-t border-line py-5 sm:grid-cols-[11rem_1fr] sm:gap-6"
                >
      ...
      <section id="contact" className="scroll-mt-20">
        <div className="shell border-t border-line py-24 text-center md:py-32">
          <Reveal>
            ...
            <div className="mt-10">
              <a
                ...
                className="inline-block border border-ink px-8 py-4 text-sm tracking-[0.12em] uppercase transition-colors duration-500 hover:border-accent hover:bg-accent hover:text-bg"
              >
                {CTA.label}
              </a>
            </div>
